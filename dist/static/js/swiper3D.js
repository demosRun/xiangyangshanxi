...
function swiperIt (cont, config) {
  ...
  this.config = config
  var _this = this
  var showSlider = _this.config.showSlider || 3
  var width = _this.config.width || 600
  var height = _this.config.height || 400
  var stepZ = _this.config.stepZ || 0.8
  var stepX = this.config.stepX || 10
  // 是否反向触摸
  var reverseTouch = this.config.reverseTouch || false
  this.styleList = []
  this.cont = cont
  ...
  for (var ind = 0; ind < this.contL.length; ind++) {
    ...
    var leftDistance = (activeLeft - (disparity * stepX)) - ((1 - scale) * this.boxWeidth)
    if (ind < activeIndex) {
  ...
  this.addEventListener(cont, 'touchstart', function (e) {
    console.log('触摸开始!')
    touchMoveX = reverseTouch ? e.touches[0].pageY : e.touches[0].pageX
    _this.isTouching = true
  })
  this.addEventListener(cont, 'touchmove', function (e) {
    touchMoveXMove = reverseTouch ? e.touches[0].pageY : e.touches[0].pageX
  })
...
swiperIt.prototype.animate = function (obj, styleList, fn) {
  setTimeout(function () {
    ...
    obj.style.transition = 'all 0.4s linear'
